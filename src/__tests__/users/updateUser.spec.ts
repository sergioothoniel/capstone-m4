...
describe("PATCH - /users", () =>{
    ...
    test("Should update an user", async () =>{
        ...
        const response = await request(app).patch(`/users/${userId}`).set("Authorization", token).send({name: 'updateTestUser'})

        expect(response.status).toBe(200)
        expect(response.body).toHaveProperty('name', 'updateTestUser')  
    ...
    test("Should throw an error when send a wrong id", async () =>{
        ...
        const response = await request(app).patch(`/users/123456}`).set("Authorization", token).send({name: 'updateTestUser'})
